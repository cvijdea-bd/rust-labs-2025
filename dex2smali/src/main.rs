...
fn print_method(dex: &Dex, method: &dex::encoded::EncodedMethod) {
    ...
    let method_type = &dex.types[method_id.proto_idx as usize];
    let method_name = &dex.strings[method_id.name_idx as usize];

    println!("      Method: {} {} {}", method_class, method_type, method_name);
}
...
fn main() {
    ...
    for class in &dex.class_defs {
        ...
        match ClassDataItem::try_parse_from_bytes(&buffer[class_data_offset..]) {
            Ok(class_data_item) => {
